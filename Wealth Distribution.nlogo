...
;-------------------------------------------------------------------------------------------------------------------------------------------------------------------
turtles-own  ;define custom turtle characteristics (variables) for all the turtles in a model
[
...
to set-initial-turtle-vars
 ...
 if (ticks > 1) [set wealth wealth + inheritance + metabolism ]  ; inherting the wealthh
   ...
   ifelse(Enable-Education)
  [set-vision] ; max-vison
  [ifelse(ticks = 0)
    [set vision 1 + random max-vision]
...
to initial-wealth-dist
  ...
  let amt-dist initial-wealth * total-wealth
  ask turtles [
...
;    output-print "red"
    ...
    if education > poor-education-max
    [ set education poor-education-max ]
]
  if color = green [
;    output-print "green"
    set education one-of ed-level + 25
    if education > middle-education-max
    [ set education middle-education-max ]
]
  if color = blue [
;    output-print "blue"
    set education one-of ed-level  + 50
    if education > rich-education-max
    [ set education rich-education-max ]
 ]
end
;-------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
ifelse (oldVersion)
 ...
 [let median-wealth median [wealth] of turtles
  ask turtles
    [ ifelse (wealth < 3 / 4 * median-wealth )
        [ set color red ]
        [ ifelse (wealth <= 1.75 * median-wealth)
            [ set color green ]
        [ set color blue ] ] ]]
...
;  output-print vision
  repeat vision
    ...
    [
;      output-print who
;      output-print "inside loop"
;      output-print how-far
;      output-print total
      set total total + [grain-here] of patch-ahead how-far  ;reports the grain-here value of the patch located at how-far patch ahead of the turtle
...
;  output-print sorted-wealths

  let total-wealth sum sorted-wealths
  let wealth-sum-so-far 0
  let index 0
  set gini-index-reserve 0
  set lorenz-points [] ; list
...
;    output-print "wealth-sum-so-far"
;    output-print wealth-sum-so-far

    set lorenz-points lput ((wealth-sum-so-far / total-wealth) * 100) lorenz-points

;    output-print "lorenz-points"
;    output-print lorenz-points
...
;    output-print "gini-index-reserve"
;    output-print gini-index-reserve
...
872
362
Number of middle class turtles
...
1042
302
Total Wealth of Poor Class
sum [wealth] of turtles with [color = red]
17
1
11
...
1042
363
Total Wealth of Middle class
sum [wealth] of turtles with [color = green]
17
1
11
...
1040
429
Total Wealth of Rich Class
sum [wealth] of turtles with [color = blue]
17
1
11
...
1787
546
Print Output inside of the code if there is any
11
0.0
1
...
2142
206
Total Wealth Per Social Class vs Time
NIL
NIL
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -5298144 true "" "plot sum [wealth] of turtles with [color = red]"
"pen-1" 1.0 0 -11085214 true "" "plot sum [wealth] of turtles with [color = green]"
"pen-2" 1.0 0 -13345367 true "" "plot sum [wealth] of turtles with [color = blue]"
